...
gamma_entropic = zeros(nsn, 2);

for i = 1:length(nrod_arr)
    
    nrod = nrod_arr(i);
    
    fname = [folder_prefix 'force_data_stalk_nrod_' int2str(nrod) '_rtmd_ini_5.91.txt'];
    ...
    if n_measure(i) ~= 0
        ...
        gamma_entropic(i, :) = numberArray(22:23);
    else
        TMD_radius(i, :) = -1;
        Zippering_force(i, :) = -1;
...
% Radial entropic force
col = col_arr(5,:);
figure;
hold on
bar(x_arr,Radial_entropic_force(:,1), 0.8,'FaceColor', col)
errorbar(x_arr,Radial_entropic_force(:,1),Radial_entropic_force(:,2)./sqrt(n_measure_tot/ncorr),'black','LineWidth',5,'Capsize',0,'LineStyle', 'none')
%errorbar(x_arr,Total_squeezing_force(:,1),Total_squeezing_force(:,2)./n_measure,'black','LineWidth',5,'Capsize',12,'LineStyle', 'none')

xlabel('# SNAREs');
ylabel('Entropic force (pN)');
ax = gca;
ax.FontSize = 16;
ax.FontWeight="bold";
ax.XMinorTick="off";
...
figure;
hold on
bar(x_arr,gamma_entropic(:,1), 0.8,'FaceColor', col)
%errorbar(x_arr,gamma_entropic(:,1),gamma_entropic(:,2)./sqrt(n_measure/ncorr),'black','LineWidth',5,'Capsize',12,'LineStyle', 'none')
errorbar(x_arr,gamma_entropic(:,1),gamma_entropic(:,2),'black','LineWidth',5,'Capsize',12,'LineStyle', 'none')

xlabel('# SNAREs');
ylabel('Entropic tension (pN/nm)');
ax = gca;
ax.FontSize = 16;
ax.FontWeight="bold";
ax.XMinorTick="off";
