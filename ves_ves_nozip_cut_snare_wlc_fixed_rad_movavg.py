...
f_name = 'ves_ves_cutsnare_rtmd_%.2f_deltah_%.2f_Nunzip_%.2f_lp_%.2f_nrod_%.0f_rod_r_%.2f_dves_%.1f_tension_%.2f_run_%.0f' % (tmd_pos_r_avg,delta_h,N_unzip,lp,n_rod,rod_bd_dia/2,2*(r_ves+hmem),tension,run)
print(f_name)
hoomd.dump.gsd(f_name+'_ini.gsd',period=None,group=hoomd.group.all(),overwrite=True)
#exit()
