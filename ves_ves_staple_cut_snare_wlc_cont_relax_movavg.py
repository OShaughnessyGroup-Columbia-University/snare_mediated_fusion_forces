...
if int(sys.argv[2])==0:
    n_bds_per_bundle = [1,1,1,1,1,1,1,1] # add N-terminal beads for VAMP and Syntaxin 
...
r_eci_arr = [8.5,8,7.5,7,6.5,6,5.5,5,4.5,4,3.5]
r_eci = r_eci_arr[(int(sys.argv[1])-1)%11]
if r_eci >= 7.0:
    delta_h = 1.0
elif r_eci >= 5:
    delta_h = 1.5
...
### Load frame for initial condition
run_number_arr = [13]
seed_number_arr = [359]
frame_number_arr = [797]
rtmd_arr = [2.27]
run_number = run_number_arr[int(sys.argv[1])//1%1]
seed_number = seed_number_arr[int(sys.argv[1])//1%1]
frame_number = frame_number_arr[int(sys.argv[1])//1%1]
rtmd = rtmd_arr[int(sys.argv[1])//1%1]

filename = 'ves_ves_avg_nostaple_rtmd_ini_%.2f_deltah_2.10_Nunzip_10.00_lp_0.50_nrod_7_rod_r_0.38_dves_55.9_tension_0.05_run_%d_%d.gsd'%(rtmd,run_number,seed_number)

snapshot = hoomd.data.gsd_snapshot(filename, frame_number)
...
# Set up moment_inertia
#tmd_pos_r = []
for p in system.particles:
    if p.type == 'D' or p.type == 'D2':
        p.moment_inertia = [i_tmd_x,i_tmd_y,i_tmd_z]
        pos = p.position
        #r_tmd = np.sqrt((pos[0]**2)+(pos[1]**2))
        #tmd_pos_r.append(r_tmd)
    elif p.type == 'A' or p.type == 'A2':
...
### Redefine the bonds since the tags of beads have been changed

# Delete Old bonds
bond_tags = []
for b in system.bonds:
    bond_tags.append(b.tag)
for z in bond_tags:
    system.bonds.remove(z)

# Re-add bonds for lipid
for p in system.particles:
    if p.type=='H':
        bd_tag=p.tag
...
for p in system.particles:
    if p.type == 'A':
        rod_body.append(p.body)
    elif p.type == 'A2':
        rod_body2.append(p.body)
    elif p.type == 'D':
        tmd_body.append(p.tag)
    elif p.type == 'D2':
        tmd_body2.append(p.tag)

for p in system.particles:
    if p.type=='LT' and p.body in tmd_body:
        tmd_tags.append(p.tag)
    elif p.type=='LT' and p.body in tmd_body2:
        tmd_tags2.append(p.tag)
    elif p.type=='LJ' and p.body in rod_body:
        rod_cterm_tags.append(p.tag)
    elif p.type=='LJI' and p.body in rod_body2:
        rod_cterm_tags2.append(p.tag)

for i in range(int(len(tmd_tags)/2)):
    tmd_tag_1 = tmd_tags[i]
    tmd_tag_2 = tmd_tags[i+int(len(tmd_tags)/2)]

    rod_tag_1 = rod_cterm_tags[2*i]
    rod_tag_2 = rod_cterm_tags[2*i+1]

    system.bonds.add('linker',rod_tag_1,tmd_tag_1)
    system.bonds.add('linker',rod_tag_2,tmd_tag_2)

for i in range(int(len(tmd_tags2)/2)):
    tmd_tag_1 = tmd_tags2[i]
    tmd_tag_2 = tmd_tags2[i+int(len(tmd_tags2)/2)]

    rod_tag_1 = rod_cterm_tags2[i]
    rod_tag_2 = rod_cterm_tags2[i+int(len(tmd_tags2)/2)]
...
f_name = 'ves_ves_cutsnare_rtmd_ini_cont_relax_%.2f_deltah_%.2f_nrod_%.0f_rod_r_%.2f_dves_%.1f_tension_%.2f_run_%.0f' % (rtmd,delta_h,n_rod,rod_bd_dia/2,2*(r_ves+hmem),tension,run)
print(f_name)
hoomd.dump.gsd(f_name+'_ini.gsd',period=None,group=hoomd.group.all(),overwrite=True)
#exit()
...
if __name__ == '__main__':
    ...
    if int(sys.argv[3])==0: 
        ...
        ld_table.bond_coeff.set('linker',func=linker_pot,rmin=0,rmax=70,coeff=dict(k=k_linker,r1=r1_linker))
        ld_table.bond_coeff.set('linker2',func=linker_pot,rmin=0,rmax=70,coeff=dict(k=k_linker,r1=r1_linker))
    else:
    ...
    bd1 = hoomd.md.integrate.langevin(group=groupROD,kT=T,seed=seed_sim)
    Gamma_snare = Gamma * 4
    ...
    N_run = 1000000 # number of timesteps, 1 timestep = 0.068 ns
