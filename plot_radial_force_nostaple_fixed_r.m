...
for irod = 1:length(nrod_arr)
    ...
    fname = [folder_prefix 'force_data_fixed_coord_nostaple_nrod_' int2str(nrod) '_rtmd_*.txt'];
    fname_pat = ['force_data_fixed_coord_nostaple_nrod_' int2str(nrod) '_rtmd_%f.txt'];
    ...
    lastNumber = cellfun(@(x) sscanf(x, fname_pat), fileNames);
    ...
    for i = 1:numFiles
        ...
        n_measure(i) = numberArray(3)*nrod;
        if n_measure(i) ~= 0
    ...
    'MarkerSize', 6, 'MarkerFaceColor', col, 'MarkerEdgeColor', col, 'LineWidth', 1, 'Color', 'k', 'CapSize', 0);
    %errorbar(r_ini(1:nmax), Radial_entropic_force(1:nmax, 1), Radial_entropic_force(1:nmax, 2), 'o', ...
